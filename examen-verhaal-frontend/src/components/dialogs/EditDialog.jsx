...
const EditDialog = ({ isOpen, onClose, isCategory = false, categories, data, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isCategory) {
        await categoriesAPI.update(data.id, formData);
      } else {
        await verhalenAPI.update(data.id, formData);
      }
      onSuccess();
      onClose();
    } catch (err) {
      ...
      <div className="bg-white rounded-lg w-[600px] mx-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex justify-between items-center p-4 border-b border-gray-200">
          <h2 className="text-xl font-mono">
            {isCategory ? "Categorie bijwerken" : "Verhaal bijwerken"}
          </h2>
          <button
            onClick={onClose}
